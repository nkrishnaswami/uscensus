...
async def aget_county_boundaries(
        session: httpx.AsyncClient,
        *,
        state: str | None = None,
        vintage: str | int = 2021,
        resolution: str = '20m') -> gpd.GeoDataFrame:
    boundaries = await _aget_boundaries(session, resolution, vintage,
                                 f'cb_{vintage}_us_county_{resolution}')
    if state:
        if re.match(r'^[A-Z]{2}$', state):
            boundaries = boundaries[boundaries['STUSPS'] == state]
...
async def aget_state_boundaries(
        session: httpx.AsyncClient,
        *,
        state: str | None = None,
        vintage: str | int = 2021,
...
async def aget_state_codes(
    ...
    territories, for vintage.
    """
    boundaries = await aget_state_boundaries(session, vintage=vintage)
    return boundaries[['STATEFP', 'STATENS', 'STUSPS', 'NAME']]
...
async def aget_county_codes(
    ...
    territories, for vintage and state, if specified.
    """
    boundaries = await aget_county_boundaries(session, vintage=vintage, state=state)
    return boundaries[['GEOID', 'STATEFP', 'STUSPS', 'STATE_NAME', 'COUNTYFP',
                       'COUNTYNS', 'NAME']]
